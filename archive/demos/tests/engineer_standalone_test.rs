// Completely standalone Engineer Character Controller test
// Run with: rustc --edition 2021 -L target/debug/deps engineer_standalone_test.rs -o engineer_test && ./engineer_test
...
// Core character types (standalone versions)
#[derive(Clone, Debug)]
...
impl CharacterPhysics {
    ...
    pub fn update_character(&mut self, state: &mut CharacterState, delta_time: f32) {
        match state.movement_mode {
            MovementMode::Fly | MovementMode::Noclip => return,
            _ => {
...
impl EngineerController {
    ...
    fn update_movement(&mut self, delta_time: f32, physics: &mut CharacterPhysics) {
        ...
        match self.state.movement_mode {
            MovementMode::Walk => self.update_ground_movement(movement_input, self.walk_speed),
            MovementMode::Run => self.update_ground_movement(movement_input, self.run_speed),
            MovementMode::Jump => self.update_jump_movement(movement_input),
    ...
    fn update_ground_movement(&mut self, input: Vector3<f32>, speed: f32) {
        if self.state.is_grounded {
            ...
            self.state.velocity.x += air_input.x * 0.016; // Assume 60fps
            self.state.velocity.z += air_input.z * 0.016;
        }
    }

    fn update_jump_movement(&mut self, input: Vector3<f32>) {
        self.update_ground_movement(input, self.walk_speed);
